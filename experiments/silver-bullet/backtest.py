...
LOT_SIZE = 10  # lots per trade ($1 P&L per point per lot)
FEES = 0.0001  # 0.01% per side (~representative CFD spread)
...
def run_backtest() -> None:
    ...
    with open(report_txt, "w", encoding="utf-8") as f:
        ...
        f.write(f"Fees            : {FEES * 100:.3f}% per side\n")
        ...
        mlflow.log_metrics(
            {
                ...
                "total_fees_paid": float(stats.get("Total Fees Paid", float("nan"))),
            }
        )
